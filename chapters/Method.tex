...
We decided to apply \emph{grid search} approach for hyperparamter optimization, which means.
We chosen key values of the hyperparameters, trained model for cases when the hyperparameters are equal to combinations of those values and checked the evaluation parameter to find the best model.\todo{???}
\medskip

We were choosing batch size values of $1,4,8$.
For the dropout probability we tried $0.5,0.7,0.9$.
Learning rate was taken with value $0.001$ and was not changed because of the nature of optimizer.
For the $L_2$ regularization used by us, for the multiplicative constant we tried several values but stopped at using single value of $10^{-8}$ because for other values the results were significantly worse. 
Also we have tried loss function with and without volume fraction component, but we put this variation as a separate experiment. 
For this we checked several values of multiplicative constant and stopped at $0.01$.s
\medskip

...
However, chosing them is important to find the best model, and in order to find them \textit{hyperparameter optimization} is used.
...
For that, we chose key values for every hyperparameters and trained model for every combination of those values and checked the evaluation parameter to find the best model.\todo{???}
\medskip

We were choosing batch size values of $1,4,8$.
For the dropout probability we tried $0.5,0.7,0.9$.\todo{may be add dependency on that}
Learning rate was taken with value $10^{-4}$, since changing value did no significantly change the outcome because of the adaptive nature of optimizer.
For the $L_2$ regularization used by us, for the multiplicative constant we tried several values but stopped at using single value of $10^{-6}$ because for other values the results were significantly worse. 

We have also tried two variation of the loss function, with and without volume fraction component, but we put this variation as a separate experiment. 
For this we checked several values of multiplicative constant and stopped at $0.2$.\todo{change that}
\medskip
...
For the most of the experiments, we have models trained for various number of optimization steps. We chosen the range of $(10, 25, 50, 75, 100)$ thousand of steps.
For some cases we took second round of optimization with respect to early stopping, taking number of iterations with steps of $5$ or $10$ thousand steps.
